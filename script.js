...
document.getElementById("correspondenciaForm").addEventListener("submit", function (e) {
  ...
  doc.rect(10, 220, 190, 30); // Ampliado a 50 para incluir instructivo
  doc.text("SEXTO DESTINATARIO:", 12, 225);
  doc.setFont("helvetica", "normal");
  doc.text(`${destinatarioNombre} - ${cargoDestinatario}`, 65, 225);
  doc.setFont("helvetica", "bold");
  doc.text("INSTRUCTIVO:", 12, 233);
  doc.setFont("helvetica", "normal");
  const instructivoTexto = doc.splitTextToSize(instructivo, 185);
  doc.text(instructivoTexto, 12, 238);
